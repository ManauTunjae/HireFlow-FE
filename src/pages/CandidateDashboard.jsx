  ...
  useEffect(() => {
    const fetchCandidateData = async () => {
      try {
        ...
        const interviewing = allApplications.filter(app => app.status?.toLowerCase() === "interview" || app.status?.toLowerCase() === "interviewing").length;
        const hired = allApplications.filter(app => app.status?.toLowerCase() === "hired").length;
        const rejected = allApplications.filter(app => app.status?.toLowerCase() === "rejected").length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans flex flex-col">
      ...
      <nav className="backdrop-blur-md border-b border-gray-200 px-4 md:px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-xs">
        ...
        <div className="flex items-center justify-between w-full sm:w-auto border-t sm:border-t-0 pt-2 sm:pt-0 border-gray-200">
          ...
          <div className="flex items-center gap-2">
            <Link to="/" className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold rounded-lg transition-colors">
              Home
            </Link>
            <button onClick={() => logout(navigate)} className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-lg transition-colors">
              Sign out
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 md:p-10 space-y-6 md:space-y-8">
        <div>
          <h2 className="text-xl md:text-2xl font-black text-gray-900 tracking-tight">Application Status</h2>
          <p className="text-xs text-gray-500 mt-1">Track your job search progress and insights in real-time analysis.</p>
        </div>

        {loading ? (
          <div className="text-center py-20 text-gray-400 animate-pulse text-sm font-medium">Analyzing database records...</div>
        ) : (
          <>
            {/* 3. REALTIDS STATISTIKKORT */}
            <section className="grid grid-cols-2 md:grid-cols-5 gap-4"> {/* 🎯 Ändrat till 5 kolumner så alla kort får plats på en rad! */}
              <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-xs flex flex-col justify-between">
                <div className="flex justify-between items-center text-gray-400">
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Total Apps</span>
                  <span className="text-emerald-500 bg-emerald-50 p-1 rounded-md text-xs">📊</span>
                </div>
                <div className="text-2xl md:text-3xl font-black text-gray-900 mt-2">{stats.total}</div>
              </div>

              <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-xs flex flex-col justify-between">
                <div className="flex justify-between items-center text-gray-400">
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Applied</span>
                  <span className="text-amber-500 bg-amber-50 p-1 rounded-md text-xs">⏳</span>
                </div>
                <div className="text-2xl md:text-3xl font-black text-gray-900 mt-2">{stats.applied}</div>
              </div>

              <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-xs flex flex-col justify-between">
                <div className="flex justify-between items-center text-gray-400">
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Interview</span>
                  <span className="text-indigo-500 bg-indigo-50 p-1 rounded-md text-xs">📈</span>
                </div>
                <div className="text-2xl md:text-3xl font-black text-gray-900 mt-2">{stats.interview}</div>
              </div>

              <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-xs flex flex-col justify-between">
                <div className="flex justify-between items-center text-gray-400">
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Hired</span>
                  <span className="text-emerald-500 bg-emerald-50 p-1 rounded-md text-xs">✅</span>
                </div>
                <div className="text-2xl md:text-3xl font-black text-emerald-600 mt-2">{stats.hired}</div>
              </div>

              <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-xs flex flex-col justify-between">
                <div className="flex justify-between items-center text-gray-400">
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Rejected</span>
                  <span className="text-red-500 bg-red-50 p-1 rounded-md text-xs">❌</span>
                </div>
                {/* 🎯 FIXAT: Nu visas stats.rejected korrekt här! 👇 */}
                <div className="text-2xl md:text-3xl font-black text-red-600 mt-2">{stats.rejected}</div>
              </div>
            </section>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border border-gray-200 rounded-xl p-5 md:p-6 shadow-xs flex flex-col justify-between min-h-[260px]">
                ...
                <div className="flex-1 flex flex-col justify-center space-y-3.5 mt-4">
                  {/* Progress bars */}
                  {[
                    { label: "Applied ⏳", value: stats.applied, color: "bg-amber-500" },
                    { label: "Interviewing 📈", value: stats.interview, color: "bg-indigo-500" },
                    { label: "Hired ✅", value: stats.hired, color: "bg-emerald-500" },
                    { label: "Rejected ❌", value: stats.rejected, color: "bg-red-500" }
                  ].map((item, idx) => (
                    <div key={idx}>
                      <div className="flex justify-between text-xs font-semibold text-gray-600 mb-1">
                        <span>{item.label}</span>
                        <span>{stats.total > 0 ? Math.round((item.value / stats.total) * 100) : 0}% ({item.value})</span>
                      </div>
            ...
            <section className="bg-white border border-gray-200 rounded-xl shadow-xs overflow-hidden">
              ...
              {myApplications.length === 0 ? (
                <div className="p-10 text-center text-xs text-gray-400">📂 You haven't applied for any jobs yet.</div>
              ) : (
                      ...
                      {myApplications.map((app) => {
                        ...
                        return (
                          <tr key={app._id} className="hover:bg-gray-50/30 transition-colors">
                            ...
                            <td className="p-4 text-gray-500">{job.company || "Pending Details"}</td>
                            <td className="p-4">
